...
def main():
    ...
    # Load the trained model for a clean evaluation
    ...
    # 4a) show first‐layer filters
    plot_conv_filters(eval_model, num_filters=6)

    # 4b) show a few activation maps for one sample
    sample, _ = next(iter(test_loader))
    img = sample[0].unsqueeze(0).to(DEVICE)  # single image
