...
def compute_hog_scratch(img, patch_size=8, orientations=9, block_size=2):
    """
    Υπολογίζει HOG χαρακτηριστικά από το μηδέν για μία εικόνα 2D np.uint8.
    Επιστρέφει το HOG διάνυσμα και το HOG visualization image.
    """
    # 1. Υπολογισμός κλίσεων
    ...
    # 2. Υπολογισμός ιστογραμμάτων για κάθε patch 8x8
    h, w = img.shape
    ...
    # 3. Block normalization (16x16, δηλ. 2x2 cells)
    eps = 1e-6
    hog_vector = []
...
def visualize_hog_comparison(img, patch_size=8, orientations=9, filename=None):
    """
    Side-by-side original and HOG visualization (from scratch).
    """
